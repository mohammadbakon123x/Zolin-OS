...
local versionLabel = "v4.0"
local BuildVersion = "v4.0"
...
local FlySpeeds = {
	{name = "Slowest", speed = 37, color = Color3.fromRGB(58, 61, 126)},     -- 1
...
local SideButtons = TranslationUI:WaitForChild("SideButtonsMenuUI") or Instance.new("Frame", TranslationUI);
SideButtons.Name = "SideButtonsMenuUI";
...
local UIAspectRatio = SideButtons:FindFirstChildOfClass("UIAspectRatioConstraint") or Instance.new("UIAspectRatioConstraint", SideButtons);
...
local ButtonCustomBeatdown = SideButtons:FindFirstChild("ButtonCustomBeatdown") or Instance.new("TextButton", SideButtons)
...
ImageLabel_ButtonSettings.ZIndex = 6
...
ImageLabel_ButtonExternalSettings.ZIndex = 6
...
ImageLabel_ButtonTeleport.ZIndex = 6
...
-- SMT
local function initializeColorCorrectionSMT()
	-- First, check for ALL existing ColorCorrection effects and clean them up
	for _, child in ipairs(game.Lighting:GetChildren()) do
		if child:IsA("ColorCorrectionEffect") and child.Name:find("CutsenseJoJo") then
			child:Destroy()
	...
	ColorCorrectionSystem.globalColorCorrection.Name = "CutsenseJoJo_Kebaited_Global"
	ColorCorrectionSystem.globalColorCorrection.Saturation = 0
	ColorCorrectionSystem.globalColorCorrection.TintColor = Color3.fromRGB(25, 20, 181)
	ColorCorrectionSystem.globalColorCorrection.Enabled = true
	...
	ColorCorrectionSystem.startTween = l__TweenService__5:Create(ColorCorrectionSystem.globalColorCorrection, TweenInfo.new(
		...
		Enum.EasingDirection.Out
			...
			Brightness = -0.25,
			Contrast = 1.03,
			TintColor = Color3.fromRGB(25, 20, 181)
		})
...
local function startColorCorrectionEffectSMT()
	-- Check for existing ColorCorrection effects FIRST
	local existingEffects = {}
	for _, child in ipairs(game.Lighting:GetChildren()) do
		if child:IsA("ColorCorrectionEffect") and child.Name:find("CutsenseJoJo") then
	...
	if not ColorCorrectionSystem.globalColorCorrection or not ColorCorrectionSystem.globalColorCorrection.Parent then
		...
		-- Reset to initial state
		ColorCorrectionSystem.globalColorCorrection.Saturation = 0
		ColorCorrectionSystem.globalColorCorrection.TintColor = Color3.fromRGB(15, 15, 15)
	end
...
local function endColorCorrectionEffectSMT()
	if not PlayedActionLIGHT then
		--print("Requested")
		for _, child in ipairs(game.Lighting:GetChildren()) do
			if child:IsA("ColorCorrectionEffect") and child.Name:find("CutsenseJoJo") then
